# 고급 Java [1] - 예외처리 & 제네릭 (Generic) & 자료구조

## 목차

- [[1] 예외의 계층 구조](#1-예외의-계층-구조)
  - [체크 예외와 언체크 예외](#체크-예외와-언체크-예외)
- [[2] 예외를 처리하는 방법](#2-예외를-처리하는-방법)
  - [폭탄 돌리기](#폭탄-돌리기)
  - [자식 예외까지 같이 처리됩니다](#자식-예외까지-같이-처리됩니다)
  - [throw 와 throws 는 다릅니다](#throw-와-throws-는-다릅니다)
- [[3] 체크 예외](#3-체크-예외)
  - [체크 예외란](#체크-예외란)
  - [장점과 단점](#장점과-단점)
- [[4] 예외를 계층으로 잡기](#4-예외를-계층으로-잡기)
  - [catch 를 계층으로 쓰기](#catch-를-계층으로-쓰기)
  - [여러 예외를 한번에 잡기](#여러-예외를-한번에-잡기)
  - [Exception 으로 던지지 마세요](#exception-으로-던지지-마세요)
  - [catch 안에서 다시 던지기](#catch-안에서-다시-던지기)
- [[5] try-with-resources](#5-try-with-resources)
  - [자원은 반드시 반납해야 합니다](#자원은-반드시-반납해야-합니다)
  - [AutoCloseable](#autocloseable)
  - [사용법](#사용법)
- [[6] 제네릭이 필요한 이유](#6-제네릭이-필요한-이유)
...
- [[11] 빅오 표기법](#11-빅오-표기법)
  ...
  - [공간복잡도](#공간복잡도)
- [[12] 배열의 시간복잡도](#12-배열의-시간복잡도)
  - [인덱스 검색과 순차검색](#인덱스-검색과-순차검색)
  - [추가와 삭제](#추가와-삭제)
- [[13] 배열의 한계와 리스트](#13-배열의-한계와-리스트)
  - [배열의 장점과 단점](#배열의-장점과-단점)
  - [해결책은 리스트](#해결책은-리스트)
- [[14] ArrayList의 단점](#14-arraylist의-단점)
  - `size` [와](#size-와-capacity) `capacity`
  - [어디에 추가하느냐가 중요합니다](#어디에-추가하느냐가-중요합니다)
- [[15] 연결리스트 (LinkedList)](#15-연결리스트-linkedlist)
  - [노드를 이어붙이는 방식](#노드를-이어붙이는-방식)
  - [그림으로 보는 연결리스트](#그림으로-보는-연결리스트)
  - [조회가 느린 이유](#조회가-느린-이유)
  - [추가와 삭제가 빠른 이유](#추가와-삭제가-빠른-이유)
  - [여기서 자주 헷갈리는 부분](#여기서-자주-헷갈리는-부분)
  - [정리](#정리)
- [[16] Set](#16-set)
  - [리스트의 한계](#리스트의-한계)
  - [Set의 특징](#set의-특징)
- [[17] Set을 직접 만들어본다면](#17-set을-직접-만들어본다면)
...
- [[19] Set의 구현체](#19-set의-구현체)
  - [LinkedHashSet](#linkedhashset)
  - [TreeSet과 트리구조](#treeset과-트리구조)
  - [균형이 깨지는 경우](#균형이-깨지는-경우)
  - [레드 블랙 트리](#레드-블랙-트리)
- [[20] Set 주요 메서드](#20-set-주요-메서드)
- [[21] Map](#21-map)
  - [key와 value](#key와-value)
  - [HashMap](#hashmap)
  - [LinkedHashMap](#linkedhashmap)
  - [TreeMap](#treemap)
- [[22] Map 주요 메서드](#22-map-주요-메서드)
- [[23] 스택과 큐, 그리고 덱](#23-스택과-큐-그리고-덱)
  - [후입선출, 스택](#후입선출-스택)
  - [선입선출, 큐](#선입선출-큐)
  - [둘 다 되는 덱](#둘-다-되는-덱)
  - [덱의 주요 메서드](#덱의-주요-메서드)
...
먼저 예외들이 어떻게 이어져 있는지부터 보고 가겠습니다

![예외 계층](media/예외-계층.png)

## 체크 예외와 언체크 예외

`Exception`의 하위 예외는 전부 체크 예외이지만, `RuntimeException`은 언체크 예외입니다

체크 예외는 반드시 잡아서 처리하거나 밖으로 던져야하고, 안 하면 컴파일 오류가 납니다

반대로 언체크 예외는 아무것도 안 해도 컴파일이 됩니다

그림에 `Error`도 같이 보이는데, 이건 메모리 부족처럼 개발자가 손쓸 수 없는 오류라서 잡으면 안 됩니다

**그래서 우리가 신경쓸건** `Exception` **아래쪽입니다**
...
처리를 하거나, 처리할 수 없으면 밖으로 던져야합니다

그러면 끝까지 아무도 안 잡으면 어떻게 될까요?

`main` 밖으로 던져지면 예외로그를 출력하면서 시스템이 종료됩니다

## 자식 예외까지 같이 처리됩니다

예외를 잡거나 던질때 자식 예외까지 같이 처리 가능합니다

예를 들어서 `Exception`을 `catch`로 잡으면 하위 예외까지 잡을 수 있습니다

예를 들어서 `Exception`을 `throws`로 던지면 하위 예외까지 던질 수 있습니다

## throw 와 throws 는 다릅니다

**이름이 비슷해서 제일 헷갈리는 부분입니다**

`throw`는 예외를 실제로 던지는 키워드입니다

`throws`는 메서드 선언부에 "이 메서드는 이런 예외를 밖으로 던질 수 있다"고 알려주는 것입니다

```java
// throws - 이 메서드가 ConnectExceptionV3 를 던질 수 있다고 알림
public void call() throws ConnectExceptionV3 {

    // throw - 여기서 실제로 던짐
    throw new ConnectExceptionV3("연결 실패");
}
```

`throw new` 처럼 붙어다니는걸 자주 보셔서 헷갈리는데, 객체를 만드는건 `new`가 하는 일이고 `throw`는 그 객체를 던지는 역할입니다
...
잡아서 처리하거나 밖으로 던지거나, 둘 중 하나를 반드시 해야하는 예외입니다

## 장점과 단점

**장점**

`throws`로 예외를 필수로 선언해야하기 때문에, 컴파일 오류를 통해 실수를 줄일 수 있습니다

**단점**

모든 예외에 대해서 처리해야 하기때문에 번거로울 수 있습니다
...
예외를 계층 구조로 만들어두면 이런식으로 처리가 가능합니다

```java
try {
    // 1. ConnectExceptionV3 발생
} catch (ConnectExceptionV3 e) {        // 2. 잡아서 처리
} catch (NetworkClientExceptionV3 e) {  // 부모
...
부모를 자식보다 먼저 쓰면 자식 `catch`는 실행될 일이 아예 없어서 컴파일 오류가 납니다

**컴파일 단계에서 걸러주니까 오히려 다행인 경우겠죠?**
...
위에 코드처럼 `|` 를 이용하여 여러 예외를 한번에 처리할 수 있습니다

이때는 공통 부모 기능만 사용가능합니다

즉, `NetworkClientExceptionV3` 의 기능만 사용 가능합니다

**계층으로 처리하면 보다 깔끔한 코드가 가능해집니다**

## Exception 으로 던지지 마세요

`Exception`은 체크 예외의 최상위 타입이라, 모든 체크 예외를 `Exception`으로 던지게 되면 중요한 예외를 체크하지 못하는 상황이 발생합니다

그래서 되도록이면 `Exception`으로 던지지 않는것이 좋습니다
...
파일이나 네트워크같은 외부자원을 사용하면 반드시 반납해야합니다

이 과정을 간편화 한게 try-with-resources 입니다

## AutoCloseable

먼저 아래와 같은 인터페이스가 있어야합니다
...
이 인터페이스를 통해 `try`가 끝날 때 자동으로 `close()` 해줍니다
...
`try` **뒤 괄호 안에서 만든 자원은** `try`**를 벗어날 때 자동으로 닫힙니다**
...
`new GenericBox<>()` 처럼 `new` 뒤에서 타입을 비워둔 `<>` 를 말합니다. 컴파일러가 왼쪽을 보고 타입을 추론해주기 때문에 비워둘 수 있는겁니다
...
이름이 비슷해서 헷갈리기 쉬운데, 이 둘은 층이 다릅니다

자료구조는 값을 어떻게 놓고 어떻게 꺼낼지에 대한 방법입니다

앞으로 볼 배열, 연결리스트, 해시 테이블, 트리가 전부 여기에 속합니다

**컬렉션 프레임워크는 자바가 그 자료구조들을 미리 구현해서 표준으로 묶어둔것입니다**

`java.util` 안에 있는 인터페이스인 `List`, `Set`, `Map` 과 그 구현체인 `ArrayList`, `HashSet`, `HashMap` 이 여기에 해당합니다
...
이렇게 규격이 먼저 있고 구현체가 그 틀 안에 들어가기 때문에 프레임워크라고 부릅니다
...
하지만 인덱스를 모르고 값으로 찾아야 한다면 이야기가 달라집니다
...
배열의 중간 위치에 추가하는것도 평균적으로 절반을 밀어야해서 `O(n/2)`의 시간이 걸리므로 결국 `O(n)`이 걸리게 됩니다
...
보시면 크기를 5로 잡아둔 배열에 6번째 값을 넣으려고 하니까 `ArrayIndexOutOfBoundsException`이 나는걸 확인할 수 있습니다
...
**즉, 컬렉션 프레임워크는 전부 제네릭으로 만들어져 있어서 앞에서 배운 내용이 그대로 이어집니다**
...
**여기서 중요한건, 이 노드들이 메모리에 나란히 붙어있을 필요가 전혀 없다는 점입니다**
...
연결리스트가 추가·삭제가 `O(1)`이라고 하는데, 정확히는 그 위치를 이미 알고 있을때만 `O(1)`입니다

```java
list.add(0, x);    // O(1) - 맨 앞은 바로 알고 있음
list.add(500, x);  // O(n) - 500번까지 걸어간 다음에야 끼워넣음
```

걸어가는데 `O(n)`이 걸리기 때문에, 결국 중간에 넣는건 `O(n)`이 되어버립니다

**정리하면 연결리스트가 진짜 빠른 경우는 맨 앞이나 맨 뒤, 그리고 순회하면서 그 자리에서 바로 넣고 뺄때입니다**
...
배열 리스트는 인덱스를 통해 추가나 삭제할 위치를 `O(1)`로 빠르게 찾습니다

하지만 추가나 삭제를 한 이후에 데이터를 한칸씩 밀어야하고, 이 부분이 `O(n)`으로 오래 걸립니다

반면에 연결 리스트는 추가나 삭제할 위치를 찾는데 `O(n)`으로 느립니다

대신 찾은 이후에는 화살표만 바꿔끼우면 되기 때문에 그 자리에서의 처리는 `O(1)`로 빠르게 끝납니다

여기서 연결 리스트의 뒤에 추가가 `O(n)`인 이유는, 맨 뒤 노드를 찾으려면 첫번째 노드부터 끝까지 걸어가야 하기 때문입니다

다만 자바의 `LinkedList`는 맨 앞 노드와 맨 뒤 노드의 주소를 둘 다 들고있어서, 뒤에 추가하는것은 `O(1)`로 처리됩니다

**정리하면 조회가 많으면 배열 리스트, 앞뒤로 넣고 빼는 일이 많으면 연결 리스트를 쓰시면 됩니다**
...
`45`를 먼저 넣었는데 출력은 `12`가 앞에 나온걸 보시면, 넣은 순서가 남지 않는다는것도 같이 확인할 수 있습니다
...
**그리고 여기서 순서가 사라지는 이유도 같이 보입니다**

넣은 순서가 아니라 나머지 값이 자리를 정하기 때문에, 순서를 보장 안해줘서 빠른게 아니라 나머지로 자리를 정한 결과 순서가 사라지는겁니다
...
**정리하면 Set이 빠른 이유는 순서를 버려서가 아니라, 나머지 연산으로 자리를 미리 계산해두기 때문입니다**
...
값을 찾을때는 결국 해시 배열을 쓰기 때문에 시간복잡도는 `HashSet`과 마찬가지로 `O(1)`입니다
...
System.out.println(set);  // [45, 12, 7] - 넣은 순서 그대로 나옵니다
```
...
System.out.println(set);  // [7, 12, 45] - 정렬된 순서로 나옵니다
```
...
앞쪽에서만 넣고 빼면 스택이 되고, 뒤로 넣어서 앞으로 빼면 큐가 되는걸 확인할 수 있습니다
...
| --------------- | --------------------- | ---------------------- | ----- | ---------------------- | --------------- |
| 배열 `int[]`      | 인덱스 `O(1)` / 값 `O(n)` | 크기가 고정이라 못합니다          | 넣은 순서 | 크기가 처음부터 정해져 있을때       | 요일별 데이터 7칸      |
| `ArrayList`     | 인덱스 `O(1)` / 값 `O(n)` | 뒤 `O(1)` / 앞·중간 `O(n)` | 넣은 순서 | 조회가 많을때 (대부분)          | 게시판 글 목록        |
| `LinkedList`    | 인덱스 `O(n)` / 값 `O(n)` | 앞뒤 `O(1)` / 중간 `O(n)`  | 넣은 순서 | 앞뒤로 자주 넣고 뺄때           | 음악 재생 목록        |
| `HashSet`       | 값 `O(1)`              | `O(1)`                 | 없음    | 중복만 없으면 될때             | 회원 아이디 집합       |
| `LinkedHashSet` | 값 `O(1)`              | `O(1)`                 | 넣은 순서 | 중복 없이 순서도 필요할때         | 최근 본 상품 목록      |
| `TreeSet`       | 값 `O(log n)`          | `O(log n)`             | 정렬 순서 | 정렬이나 범위 검색이 필요할때       | 점수 순위표          |
| `HashMap`       | key `O(1)`            | `O(1)`                 | 없음    | key로 값을 찾을때 (대부분)      | 회원 `<ID, 회원정보>` |
| `LinkedHashMap` | key `O(1)`            | `O(1)`                 | 넣은 순서 | key-value에 순서도 필요할때    | 최근 검색 기록        |
| `TreeMap`       | key `O(log n)`        | `O(log n)`             | 정렬 순서 | key 정렬이나 범위 검색이 필요할때   | 사전 `<단어, 뜻>`    |
| `Stack`         | 맨 위만 `O(1)`           | 한쪽 끝 `O(1)`            | 후입선출  | 마지막에 넣은것부터 꺼낼때(쓰지마세요!) | 브라우저 뒤로가기       |
| `Queue`         | 맨 앞만 `O(1)`           | 뒤로 넣고 앞에서 빼기 `O(1)`    | 선입선출  | 들어온 순서대로 처리할때          | 수강신청 대기열        |
| `Deque`         | 양쪽 끝만 `O(1)`          | 양쪽 끝 `O(1)`            | 넣은 순서 | 스택과 큐를 둘 다 써야할때        | n개만 유지하는 최근 기록  |


`TreeSet`과 `TreeMap`의 범위 검색은 "80점 이상만 뽑기" 처럼 구간으로 잘라서 꺼내는걸 말합니다

그리고 `Stack`, `Queue`, `Deque` 는 셋 다 `ArrayDeque`로 만드는게 가장 좋습니다

**정리하면 특별한 이유가 없으면** `ArrayList`**와** `HashMap`**을 쓰고, 순서나 정렬이 필요할때만 다른걸 고르시면 됩니다**
...
`[10]`에서 규격이 먼저 있고 구현체가 그 틀 안에 들어간다고 말씀드렸는데, 그게 이 그림입니다

위쪽 둥근 상자가 규격을 정하는 인터페이스고, 아래쪽 네모 상자가 실제로 쓰는 구현체입니다

여기서 하나 짚고 갈 점은 `Map`은 `Collection` 소속이 아니라 따로 있다는 점입니다

`Collection`은 값을 하나씩 담는 규격인데 `Map`은 key-value 쌍을 담기 때문에, 애초에 규격 자체가 다릅니다

그리고 `LinkedList`는 `List`이면서 `Deque`이기도 해서, 리스트로도 쓰고 덱으로도 쓸 수 있습니다

---

**여기까지 따라오시느라 고생 많으셨습니다!**

예외처리부터 시작해서 제네릭, 그리고 자료구조까지 쉬지않고 달려왔는데요

한번에 다 외우실 필요는 전혀 없고, 나중에 필요하실 때 `[24]`의 정리표만 다시 보셔도 충분합니다
